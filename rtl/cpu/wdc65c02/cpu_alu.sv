		...
		case (op)
			ALU_ADC: begin
				if (decimal_add) begin
					...
					carry_out = d_hi_c;
					// V is the overflow of the intermediate sum before the
					// high-nibble +6, i.e. bit 7 of (hi<<4 | adjusted lo).
					// d_hi[3] is exactly that bit. Verified against the
					// SingleStepTests 65x02 WDC/Rockwell/Synertek ADC sets
					// (0 mismatches; the +6-corrected bit missed ~26%).
